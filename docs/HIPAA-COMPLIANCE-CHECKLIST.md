...
1. **DB-level BEFORE triggers** — `audit_logs_reject_update`, `audit_logs_reject_delete`, `audit_logs_reject_truncate` each invoke `audit_logs_reject_mutation()` and `RAISE EXCEPTION ... ERRCODE 'insufficient_privilege'`. Any attempt to run `UPDATE`, `DELETE`, or `TRUNCATE` fails at the database even if issued via raw SQL or a privileged client. (See `prisma/migrations/20260417100556_init/migration.sql`.)
